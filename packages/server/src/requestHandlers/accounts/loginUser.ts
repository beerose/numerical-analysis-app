...
export type LoginUserResponse = BackendResponse<{
  token: string;
  user: UserDTO;
  privileges?: UserPrivileges;
}>;

export const loginUser = (req: Request, res: LoginUserResponse) => {
  handleBadRequest(LoginUserBodyV, req.body, res).then(({ email, password }) =>
    doLoginUser(email, password, res)
  );
};
